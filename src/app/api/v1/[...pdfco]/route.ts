...
async function proxyRequest(req: NextRequest, { params }: { params: Promise<{ pdfco: string[] }> }) {
    try {
        ...
        if (userApiKey === 'dn_test_sandbox') {
            ...
            userId = user?.id || null;
            if (!userId) {
                return NextResponse.json({ error: "Unauthorized sandbox access. Please log in." }, { status: 401 });
            }
        } else {
        ...
        // 4. Prepare the headers for the PDF.co request
        ...
        // 7. Return the PDF.co response exactly as it is received
        const responseHeaders = new Headers(proxyResponse.headers);
        responseHeaders.delete('x-api-key');

        // 8. If successful (2xx), deduct credits and log usage
